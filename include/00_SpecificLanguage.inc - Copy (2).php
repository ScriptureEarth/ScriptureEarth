	...
	if ($result2) {
		while ($r2 = $result2->fetch_array()) {
			...
			<tr>
				<td style='width: 45px; '>
					<?php
					echo "<div class='linePointer' title='" . translate('Download the cell phone module for', $st, 'sys') . "$Cell_Phone_Title' onclick='CellPhoneModule(\"$st\", \"$ISO\", \"$ROD_Code\", \"$Cell_Phone_File\")'><img class='iconActions' src='../images/android_module-icon.jpg' alt='".translate('Cell Phone', $st, 'sys')."' title='".translate('Cell Phone', $st, 'sys')."' />";
				echo "</td>";
				echo "<td>";
					echo "<div class='linePointer' title='" . translate('Download the cell phone module for', $st, 'sys') . "$Cell_Phone_Title' onclick='CellPhoneModule(\"$st\", \"$ISO\", \"$ROD_Code\", \"$Cell_Phone_File\")'><span class='lineAction'>" . translate('Download', $st, 'sys') . "</span> " . translate('the cell phone module for', $st, 'sys') . ' ' . ($Cell_Phone_Title == 'Android App' ? 'Android' : $Cell_Phone_Title);
				echo ' ' . $optional . '</div>';
					?>
				</td>
			</tr>
                    ...
                    <tr>
                        ...
                        for (; $i < count($VideoConvertContents); $i++) {										// continue using $i to iterate through $VideoConvertContents
                            ...
                            <td colspan='<?php echo $col_span; ?>' style='vertical-align: top; width: <?php echo $num_array_col; ?>; '>
                                ...
                                echo "<input type='checkbox' id='DVideoPlaylist_${z}_$video_download_index' name='DownloadVideoPlaylist_$z' value='$video_download_index' onclick='DownloadVideoPlaylistClick(\"$video_download_index\", $ZipFile, $z)' />&nbsp;&nbsp;<span style='font-size: 11pt; font-weight: normal; '>$VideoConvertWithTab[1]</span>";
                                ?>
                            </td>
                            <?php
                            $j++;
			...
			<tr>
				<td style='width: 45px; '>
					...
					if ($Cell_Phone_Title == 'MySword (Android)')
						...
						echo "<div class='linePointer' title='" . translate('Download the cell phone module for', $st, 'sys') . "$Cell_Phone_Title' onclick='CellPhoneModule(\"$st\", \"$ISO\", \"$ROD_Code\", \"$Cell_Phone_File\")'><img class='iconActions' src='../images/CellPhoneIcon.png' alt='".translate('Cell Phone', $st, 'sys')."' title='".translate('Cell Phone', $st, 'sys')."' />";
					echo "</div>";
				echo "</td>";
				echo "<td>";
					if ($Cell_Phone_Title == 'MySword (Android)')
						...
						echo "<div class='linePointer' title='" . translate('Download the cell phone module for', $st, 'sys') . "$Cell_Phone_Title' onclick='CellPhoneModule(\"$st\", \"$ISO\", \"$ROD_Code\", \"$Cell_Phone_File\")'><span class='lineAction'>" . translate('Download', $st, 'sys') . "</span> " . translate('the cell phone module for', $st, 'sys') . " $Cell_Phone_Title";
					echo ' ' . $optional . '</div>';
					?>
				</td>
			</tr>
...
if ($PlaylistAudio && $Internet) {
	...
	while ($r_Playlist = $result_Playlist->fetch_array()) {
		...
		<tr>
			...
			echo "</td>";
			echo "<td>";
				echo "<div class='linePointer' title='".translate('Download', $st, 'sys')." $PlaylistAudioTitle' onclick='PlaylistTableClick_$z()'><span class='lineAction'>".translate('Download', $st, 'sys').":</span> ". $PlaylistAudioTitle ."</div>";
			...
			while ($r2 = $result2->fetch_array()) {
				...
				<tr>
                    ...
                    <!--div class='lineItem' style='margin-top: -19px; margin-bottom: -16px; '-->
                        ...
                        echo "<div class='linePointer' title='$LN: ".translate('Download module.', $st, 'sys')."' onclick='Study(\"$st\", \"$ISO\", \"ROD_Code\", \"$ScriptureURL\")'>";
