	...
	if ($result2 && $num > 0) {
		...
		<tr>
			<td>
				<?php
				$SAB_Book = array();
				$SAB_Book_Chapter = array();
				$a_index = 0;
				echo "<a href='#' onclick='GoSAB(document.form_SAB_Chapters.NT_SAB_Chapters)'><img class='iconActions' src='images/SAB-readListen-icon.png' alt='".translate('Read and Listen', $st, 'sys')."' title='".translate('Read and Listen', $st, 'sys')."' />";
				echo "</a>";
			echo "</td>";
			echo "<td>";
				?>
				...
				<?php
				echo "<a href='#' onclick='GoSAB(document.form_SAB_Chapters.NT_SAB_Chapters)'><span class='lineAction'>" . translate('Synchronized text and audio', $st, 'sys') . "</span></a>: ";
				?>
				<div id='SABSelects' style='display: inline; '>
				<?php
				/*if ($SABLink == 1)
					echo translate('to the New Testament', $st, 'sys');
				else if ($SABLink == 2)
				...
				*/
				...
				echo "<select id='NT_SAB_Book' name='NT_SAB_Book' class='selectOption' onchange='SABChangeChapters(\"NT\", \"$ISO\", \"ROD_Code\", this.options[this.selectedIndex].value); GoSAB(document.form_SAB_Chapters.NT_SAB_Chapters)'>";
				foreach ($NT_array[NT_EngBook] as $a) {											// display the NT books in the MAJOR language!
					...
					if ($result_array && $num_array > 0) {
						...
						while ($r_array = $result_array->fetch_array()) {						// display the chapters
							...
							if (!empty($Book_Chapter_HTML)) {
								$SAB_Chapter = trim($r_array['SAB_Chapter']);
								$SAB_Book_Chapter[$a_index][] = $SAB_Chapter;
								$j = $j . "," . $SAB_Chapter . "," . $Book_Chapter_HTML;
							}
						}
						echo "<option id='SAB_Book_$a_index' name='SAB_Book_$a_index' class='selectOption' value='$j'>$a</option>";
					}
					$a_index++;
				}
				echo "</select>";
				...
				?>
				<form name='form_SAB_Chapters' id='form_SAB_Chapters' style='display: inline; '>
				<select name='NT_SAB_Chapters' id='NT_SAB_Chapters' class='selectOption' onchange='GoSAB(document.form_SAB_Chapters.NT_SAB_Chapters)'>
				<?php
				$a_index = 0;
				foreach ($NT_array[NT_EngBook] as $a) {											// display the NT books in the MAJOR language!
					$query_array="SELECT * FROM SAB WHERE ISO_ROD_index = '$ISO_ROD_index' AND SAB_Book = '".($a_index+41)."' AND (Book_Chapter_HTML is not null AND trim(Book_Chapter_HTML) <> '') ORDER BY SAB_Chapter ASC";
					...
					if ($result_array && $num_array > 0) {
						...
						while ($r_array = $result_array->fetch_array()) {						// display the chapters
							...
							if (!empty($Book_Chapter_HTML)) {
								$SAB_Chapter = trim($r_array['SAB_Chapter']);
								echo "<option id='SAB_Chapters_$i' name='SAB_Chapters_$i' class='selectOption' value='$a^data/$ISO/sab/$Book_Chapter_HTML'>$SAB_Chapter</option>";
							}
							$i++;
						}
						break;
