...
*/
	$query="SELECT Book_Chapter_HTML FROM SAB WHERE ISO_ROD_index = '$ISO_ROD_index' LIMIT 1";			// test to see if OT is there. actually have html files in sab
	$result2=$db->query($query);
	$num=$result2->num_rows;
	if ($result2 && $num > 0) {
		$r_array = $result2->fetch_array(MYSQLI_ASSOC);
		$Book_Chapter_HTML = trim($r_array['Book_Chapter_HTML']);
//echo '# '. $Book_Chapter_HTML . ' #<br />';
		$temp_Book_Chapter_HTML = '';
		$subfolder = '';
		if (preg_match('/^([a-z]{3}([A-Z0-9])?(?(2)[_A-Za-z0-9]*|))/', $Book_Chapter_HTML, $match)) {	// gives ISO ( + year date or LETTERS). If Clause: (?(2)[_A-Za-z0-9]*|) Evaluate the condition below and proceed accordingly (2) checks whether the 2nd capturing group matched when it was last attempted
			$temp_Book_Chapter_HTML = $match[1];														// {ISO}Zzzz
		}
		else {
			// leave at '' ($temp_Book_Chapter_HTML)
			//$temp_Book_Chapter_HTML = substr($Book_Chapter_HTML, 0, strpos($Book_Chapter_HTML, '-'));	// e.g. Kayah
		}
//echo '# '. $temp_Book_Chapter_HTML . ' #<br />';

		if (is_dir("./data/$ISO/sab/$temp_Book_Chapter_HTML")) {
			$subfolder = $temp_Book_Chapter_HTML;
		}
		//elseif (is_dir("./data/$ISO/sab/")) {
		//	$subfolder = '';
		//}
		//else {																							// = ..data/[ISO]/sab (just normal)
		//}
//echo '# ' . $subfolder . ' #<br />';
...
//echo '1<br />';
		...
		elseif ($temp_Book_Chapter_HTML != '' && file_exists("./data/$ISO/sab/".$subfolder."/js/book-names.js")) {
			$SAB_Read = file("./data/$ISO/sab/".$subfolder."/js/book-names.js");						// read the 'book-names.js' file
//echo '2<br \>';
			// books => name: and ref:
			foreach ($SAB_Read as $line_num => $line) {													// read through the lines
				$ln = '';
		...
		else {		// = ./data/[ISO]/sab/[ISO]/".$subfolder."/js/book-names.js
//echo '3<br />';
		}
			...
			*/
			...
			echo '<tr>';
				...
				echo '<td>';
					...
					echo "<select id='SAB_Book' name='SAB_Book' class='selectOption' onchange='GoSAB_subfolder(\"$ISO\", this.options[this.selectedIndex].value, \"$subfolder\")'>";
