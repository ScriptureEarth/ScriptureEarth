...
if (!$result || (mysql_num_rows($result) < 1)) {
	die ("<body>&ldquo;$ISO&rdquo; " . translate('is not found.', $st, 'sys') . '</body></html>');
}
...
if ($NT_Audio > 0 || $OT_Audio > 0 || $PDF_NT > 0 || $PDF_OT > 0 || $SB_PDF > 0) {							// if it is a 1 then
	...
	if ($OT_Audio > 0) {
		...
		?>
		<div id='OT_Download_MB' style='float: right; vertical-align: bottom; margin-top: 8px; '></div>
		</td></tr>
        <?php
		$a_index = 0;
		$j = 0;
	...
	if ($NT_Audio > 0) {
		...
		?>
		<div id='NT_Download_MB' style='float: right; vertical-align: bottom; margin-top: 8px; '></div>
		</td>
        </tr>
        <?php
		$a_index = 0;
...
if ($study || $viewer) {
	if ($viewer) {
		$query="SELECT viewer_ROD_Variant FROM viewer WHERE ISO_ROD_index = '$ISO_ROD_index' AND Variant_Code = '$Variant_Code'";						// check if there is a viewer
		...
		<p class='lineItem'>
        <?php
		echo "<a href='./viewer/views.php?iso=$ISO&ROD_Code=$ROD_Code&Variant_Code=$Variant_Code&ROD_Var=$ROD_Var&st=$st' title='".translate('Viewer for $LN', $st, 'sys')."' target='_blank'><img class='iconActions' align='middle' src='images/study-icon.jpg' alt='".translate('Study', $st, 'sys')."' title='".translate('Study', $st, 'sys')."' width='45' height='40' />";
			...
			if (mysql_num_rows($result2) >= 1) {
				...
				while ($i < $num) {
                    ...
                    <div class='lineItem' style='margin-top: -19px; margin-bottom: -16px; '>
						...
						echo "</a>";
						?>
						</p><p style='display: inline-block; vertical-align: middle; width: 730px; font-weight: bold; color: navy; font-size: 1em; '>				
						<?php
						echo "<a href='#' title='$LN: ".translate('Download module.', $st, 'sys')."' onclick='Study(\"$st\", \"$ISO\", \"ROD_Code\", \"$ScriptureURL\")'>";
...
if ($links) {
	...
	$query="SELECT * FROM links WHERE ISO_ROD_index = '$ISO_ROD_index' AND map = 0 AND buy = 0 AND BibleIs = 0";
	$result2=mysql_query($query);
	if (isset($result2)) {
		if (mysql_num_rows($result2) >= 1) {
			$num=mysql_num_rows($result2);
