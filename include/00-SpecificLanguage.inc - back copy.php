...
//echo '#'.$ISO_ROD_index.'#<br />';
	$SABindex=0;
	$query="SELECT subfolder, description, pre_scriptoria FROM SAB_scriptoria WHERE ISO_ROD_index = '$ISO_ROD_index'";		// parent table to SAB table
...
//echo $preScriptoria . '<br />';
			if ($preScriptoria != '') {																			// field set with preScriptoria
...
echo $c . ') would have UPDATE CellPhone<br />';
			...
			<tr>
				<td style='width: 45px; '>
					<?php
					echo "<div class='linePointer' title='" . translate('Download the Scripture Earth app for iOS', $st, 'sys') . "' onclick='CellPhoneModule(\"$st\", \"$ISO\", \"$ROD_Code\", \"$Cell_Phone_File\")'><img class='iconActions' src='../images/iOS_App.jpg' alt='".translate('Cell Phone', $st, 'sys')."' title='".translate('Cell Phone', $st, 'sys')."' />";
				echo "</td>";
				echo "<td>";
					echo "<div class='linePointer' title='" . translate('Download the Scripture Earth app for iOS', $st, 'sys') . "' onclick='CellPhoneModule(\"$st\", \"$ISO\", \"$ROD_Code\", \"$Cell_Phone_File\")'><span class='lineAction'>" . translate('Download', $st, 'sys') . "</span> " . translate('the Scripture Earth app for iOS', $st, 'sys');
				echo ' ' . $optional . '</div>';
					?>
				</td>
			</tr>
...
/*
	*************************************************************************************************************
		Is it a cell phone module (apart from the Android App)?
	*************************************************************************************************************
*/
	/*
		GoBible (Java)
	...
	*/
if ($CellPhone) {
	$query="SELECT * FROM CellPhone WHERE ISO_ROD_index = '$ISO_ROD_index' AND (Cell_Phone_Title <> 'Android App' AND Cell_Phone_Title <> 'Apple App')";
	$result2=$db->query($query);
	if ($result2) {
		while ($r2 = $result2->fetch_array(MYSQLI_ASSOC)) {
			$Cell_Phone_Title=$r2['Cell_Phone_Title'];
