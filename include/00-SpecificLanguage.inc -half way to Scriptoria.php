...
*/
	$query="SELECT subfolder, description, pre_scriptoria FROM SAB_scriptoria WHERE ISO_ROD_index = '$ISO_ROD_index'";
	...
	if ($result_sub && $num_sub > 0) {
		while ($row_sub=$result_sub->fetch_array(MYSQLI_ASSOC)) {
			...
			$pre_scriptoria=trim($row_sub['pre_scriptoria']);
			$testFileName = '';
			if (is_dir("./data/$ISO/".$subfolder)) {
			}
			else {
				continue;
			}
			
			if (preg_match('/\/(.*)\/$/', $subfolder, $match)) {
				$testFileName = $match[1];
			}
			else {
				//echo 'This is not suppossed to happen. subfolder field of SAB_scriptoria table does not have /(.*)/$ in '.$ISO.'.<br />';
				$testFileName = '';
				$query="SELECT Book_Chapter_HTML FROM SAB WHERE ISO_ROD_index = '$ISO_ROD_index' LIMIT 1";
				$result_sab=$db->query($query);
				$num_sab=$result_sab->num_rows;
				if ($result_sab && $num_sab > 0) {
					$row_sab=$result_sab->fetch_array(MYSQLI_ASSOC);
					$Book_Chapter_HTML=trim($row_sab['Book_Chapter_HTML']);
					$testFileName = substr($Book_Chapter_HTML, 0, strpos($Book_Chapter_HTML, '-'));
					if ($ISO == $testFileName) {
						$testFileName = '';
					}
//echo 'testFileName: #'.$testFileName.'#<br />';
				}
				else {
					echo "Opps. There are no records in SAB table for ISO_ROD_index $ISO_ROD_index.<br />";
				}
			}
			...
			if (file_exists("./data/$ISO/sab/js/".$testFileName."book-names.js")) {							// not on the PHP server but my office/home oomputer
				$SAB_Read = file("./data/$ISO/sab/js/".$testFileName."book-names.js");						// read the '[ISO][SABnum]-book-names.js' file
				//echo '1<br />';
			...
			elseif (file_exists("./data/$ISO/".$subfolder."js/book-names.js")) {							// ./data/$ISO/sab/". the subfolder \/(.*)\/$ ."/js/book-names.js
			...
			if ($subfolder == 'sab/') {
						...
						echo "<td>";
							...
							<div class='SABReadListen'>
								...
								<div id='OTSABSelects' style='display: inline; '>
									...
									echo "<select id='OT_SAB_Book' name='OT_SAB_Book' class='selectOption' onchange='if (this.options[this.selectedIndex].text != \"".translate('Choose One...', $st, 'sys')."\") { GoSAB(\"$ISO\", this.options[this.selectedIndex].value, \"$subfolder\"); }'>";
				...
				*/
						...
						echo "<td>";
							...
							<div class='SABReadListen'>
								...
								<div id='NTSABSelects' style='display: inline; '>
									...
									echo "<select id='NT_SAB_Book' name='NT_SAB_Book' class='selectOption' onchange='if (this.options[this.selectedIndex].text != \"".translate('Choose One...', $st, 'sys')."\") { GoSAB(\"$ISO\", this.options[this.selectedIndex].value, \"$subfolder\"); }'>";
									echo "<option>".translate('Choose One...', $st, 'sys')."</option>";
				...
				echo '<tr>';
					echo '<td>';
						echo "<a href='#' onclick='window.open(\"data/$ISO/${subfolder}index.html\", \"SABPage\");'><img class='iconActions' ";
					...
					echo '</td>';
					echo '<td>';
						echo "<a href='#' onclick='window.open(\"data/$ISO/${subfolder}index.html\", \"SABPage\");'><span ";
						echo "class='lineAction'>" . translate('Text (with audio and video where available)', $st, 'sys') . "</span></a>: ";
						echo "<form name='form_SAB_Books' id='form_SAB_Books' style='display: inline; '>";
						echo "<select id='SAB_Book' name='SAB_Book' class='selectOption' onchange='if (this.options[this.selectedIndex].text != \"".translate('Choose One...', $st, 'sys')."\") { GoSAB_subfolder(\"$ISO\", this.options[this.selectedIndex].value, \"$subfolder\"); }'>";
						echo "<option>".translate('Choose One...', $st, 'sys')."</option>";
						foreach ($SAB_lists as $main_key => $SAB_value) {									// display the NT books in the MAJOR language! $main_key used in option element.
							//echo $SAB_value.'|';
							//var_dump($NT_array[5]);
							$t = 0;
							foreach ($NT_array[5] as $key => $SAB_line) {									// iteracte through NT (NT book abbreviation)
								//echo $SAB_line . '<br />';
							...
							if ($t == 1) {
								...
								echo "<option id='SAB_Book_".$line."' name='SAB_Book_".$line."' class='speaker' value='".$main_key."'>".translate($line, $st, 'sys')."</option>";
							}
						}
						echo "</select>";
						echo "</form>";
