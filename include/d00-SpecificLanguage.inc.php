...
if ($SAB) {
	...
	*/
	...
	$SAB_OT_lists = [];
	$SAB_NT_lists = [];
	if (file_exists("../data/$ISO/sab/js/".$temp_Book_Chapter_HTML."book-names.js")) {
	//$file_headers = @get_headers("https://s3.amazonaws.com/scriptureearth/data/$ISO/sab/js/".$temp_Book_Chapter_HTML."book-names.js");	// Fetches all 9 of the headers sent by the server in response to an HTTP request.
	//$file_headers0 = ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily');
	//if ($temp_Book_Chapter_HTML != '' && $file_headers0) {																		// not on the PHP server but my office/home oomputer
	...
	if ($result2 && $num > 0) {
		while ($r_links = $result2->fetch_array()) {
			...
			<tr>
				<td>
					<?php
					echo "<a href='#' onclick='LinkedCounter(\"BibleIs_".$counterName."_".$GetName."_".$ISO."\", \"".$URL."\")'><img class='iconActions' src='images/BibleIs-icon.jpg' alt='".translate('Read', $st, 'sys')."' title='".translate('Read', $st, 'sys')."' />";
					echo "</a>";
				echo "</td>";
				echo "<td>";
        		...
        		<tr>
					<td>
						...
						if ($Item == 'B') {
							...
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$whole_Bible' title='".translate('Read the Bible.', $st, 'sys')."' target='_blank'><img class='iconActions' src='images/read-icon.jpg' alt='".translate('Read', $st, 'sys')." (PDF)"."' title='".translate('Read', $st, 'sys')."' />";
							echo "</a>";
							echo "</td>";
							echo "<td>";
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$whole_Bible' title='".translate('Read the Bible.', $st, 'sys')."' target='_blank'><span class='lineAction'>".translate('Read', $st, 'sys')."</span> ".translate('the Bible', $st, 'sys')." (PDF)"."</a>";
						}
						else {
							$complete_Scripture=trim($r_SB['Scripture_Bible_Filename']);
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$complete_Scripture' title='".translate('Read a Scripture portion.', $st, 'sys')."' target='_blank'><img class='iconActions' src='images/read-icon.jpg' alt='".translate('Read', $st, 'sys')." (PDF)"."' title='".translate('Read', $st, 'sys')."' />";
							echo "</a>";
							echo "</td>";
							echo "<td>";
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$complete_Scripture' title='".translate('Read a Scripture portion.', $st, 'sys')."' target='_blank'><span class='lineAction'>".translate('Read', $st, 'sys')."</span> ".translate('a Scripture portion', $st, 'sys')." (PDF)"."</a>";
						}
						?>
					</td>
				</tr>
			...
			if ($result1 && $num > 0) {
				if ($r1 = $result1->fetch_array()) {
					...
					<tr>
						<td>
							<?php
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$OT_PDF_Filename' title='".translate('Read the Old Testament.', $st, 'sys')."' target='_blank'><img class='iconActions' src='images/read-icon.jpg' alt='".translate('Read', $st, 'sys')." (PDF)"."' title='".translate('Read', $st, 'sys')."' />";
							echo "</a>";
						echo "</td>";
						echo "<td>";
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$OT_PDF_Filename' title='".translate('Read the Old Testament.', $st, 'sys')."' target='_blank'><span class='lineAction'>".translate('Read', $st, 'sys')."</span> ".translate('the Old Testament', $st, 'sys')." (PDF)"."</a>";
							?>
						</td>
					</tr>
					<?php
						...
						<tr>
							...
							echo "<td>";
								...
								foreach ($OT_array[OT_EngBook] as $a) {									// there is/are book(s)
									...
									if ($r_array = $result_array->fetch_array()) {
										...
										if (!empty($OT_PDF_Filename)) {
											echo "<option class='selectOption' id='OT_PDF_Media_$a' value='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$OT_PDF_Filename'>$a</option>";
										}
									}
									$a_index++;
								}
								...
								if ($r_array = $result_array->fetch_array()) {
									$OT_PDF_Filename = trim($r_array['OT_PDF_Filename']);
									if (!empty($OT_PDF_Filename)) {
										echo "<option class='selectOption' value='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$OT_PDF_Filename'>".translate('Appendix', $st, 'sys')."</option>";
									}
								}
								...
								if ($r_array = $result_array->fetch_array()) {
									$OT_PDF_Filename = trim($r_array['OT_PDF_Filename']);
									if (!empty($OT_PDF_Filename)) {
										echo "<option class='selectOption' value='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$OT_PDF_Filename'>".translate('Glossary', $st, 'sys')."</option>";
									}
								}
								echo "</select>";
								echo "</form>";
			...
			if ($result1 && $num > 0) {
				if ($r_NT = $result1->fetch_array()) {
					...
					<tr>
						<td>
							<?php
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$NT_PDF_Filename' title='".translate('Read the New Testament.', $st, 'sys')."' target='_blank'><img class='iconActions' src='images/read-icon.jpg' alt='".translate('Read', $st, 'sys')." (PDF)"."' title='".translate('Read', $st, 'sys')."' />";
							echo "</a>";
						echo "</td>";
						echo "<td>";
							echo "<a href='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$NT_PDF_Filename' title='".translate('Read the New Testament.', $st, 'sys')."' target='_blank'><span class='lineAction'>".translate('Read', $st, 'sys')."</span> ".translate('the New Testament', $st, 'sys')." (PDF)"."</a>";
							?>
						</td>
					</tr>
					<?php
						...
						<tr>
							...
							echo "<td>";
								...
								foreach ($NT_array[NT_EngBook] as $a) {									// there is/are book(s) (from NT_Books.php)
									...
									if ($r_array = $result_array->fetch_array()) {
										...
										if (!empty($NT_PDF_Filename)) {
											echo "<option class='selectOption' value='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$NT_PDF_Filename'>$a</option>";
										}
									}
									$a_index++;
								}
								...
								if ($r_array = $result_array->fetch_array()) {
									$NT_PDF_Filename = trim($r_array['NT_PDF_Filename']);
									if (!empty($NT_PDF_Filename)) {
										echo "<option class='selectOption' value='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$NT_PDF_Filename'>".translate('Appendix', $st, 'sys')."</option>";
									}
								}
								...
								if ($r_array = $result_array->fetch_array()) {
									$NT_PDF_Filename = trim($r_array['NT_PDF_Filename']);
									if (!empty($NT_PDF_Filename)) {
										echo "<option class='selectOption' value='https://s3.amazonaws.com/scriptureearth/data/$ISO/PDF/$NT_PDF_Filename'>".translate('Glossary', $st, 'sys')."</option>";
									}
								}
								echo "</select>";
								echo "</form>";
...
if ($CellPhone) {
	...
	if ($result2) {
		while ($r2 = $result2->fetch_array()) {
			...
			//$file_headers = @get_headers('https://s3.amazonaws.com/scriptureearth/data/' . $ISO . '/study/' . $Cell_Phone_File);	// Fetches all 9 of the headers sent by the server in response to an HTTP request.
			//if ($file_headers[0] == 'HTTP/1.1 404 Not Found' || $file_headers[0] == 'HTTP/1.1 302 Moved Temporarily') {
				...
				//echo $matches[1] . '<br />
				$list = [];
				$list = glob('https://s3.amazonaws.com/scriptureearth/data/' . $ISO . '/study/' . $matches[1] . '*.apk');			// server (glob = find a file based on wildcards)
				if (empty($list)) {
			...
			*/
			if ($OT_Audio) {
				...
				if ($result2) {
					...
					$a_index = 0;
					echo "<a href='#' title='".translate('Listen to the Old Testament.', $st, 'sys')."' onclick='ListenAudio(document.form_OT_Chapters_mp3.OT_Chapters_mp3, true, \"OTListenNow\", $OTNT)'><img  class='iconActions' src='images/listen-icon.jpg' alt='".translate('Listen', $st, 'sys')."' title='".translate('Listen', $st, 'sys')."' />";
					echo "</a>";
					echo "</td>";
					echo "<td>";
					?>
					<div class='OTAudio'>
					<?php
					echo "<a href='#' title='".translate('Listen to the Old Testament.', $st, 'sys')."' onclick='ListenAudio(document.form_OT_Chapters_mp3.OT_Chapters_mp3, true, \"OTListenNow\", $OTNT)'><span class='lineAction'>".translate('Listen', $st, 'sys')."</span></a> ".translate('to the audio Old Testament:', $st, 'sys');
					?>
					...
					foreach ($OT_array[OT_EngBook] as $a) {											// display the OT books in the MAJOR language!
						...
						if ($result_array && $num_array > 0) {
							...
							while ($r_array = $result_array->fetch_array()) {						// display the chapters using a drop-down box
								...
								if (!empty($OT_Audio_Filename)) {
									$OT_Audio_Chapter = trim($r_array['OT_Audio_Chapter']);
									echo "<option id='OT_Audio_Chapters_$i' name='OT_Audio_Chapters_$i' class='selectOption' value='$a^https://s3.amazonaws.com/scriptureearth/data/$ISO/audio/$OT_Audio_Filename'>$OT_Audio_Chapter</option>";
								}
								$i++;
							}
							break;
			...
			if ($NT_Audio) {
				...
				if ($result2) {
					...
					$a_index = 0;
					echo "<a href='#' title='".translate('Listen to the New Testament.', $st, 'sys')."' onclick='ListenAudio(document.form_NT_Chapters_mp3.NT_Chapters_mp3, true, \"NTListenNow\", $OTNT)'><img class='iconActions' src='images/listen-icon.jpg' alt='".translate('Listen', $st, 'sys')."' title='".translate('Listen', $st, 'sys')."' />";
					echo "</a>";
					echo "</td>";
					echo "<td>";
					?>
					<div class='NTAudio'>
                    <?php
					echo "<a href='#' title='".translate('Listen to the New Testament.', $st, 'sys')."' onclick='ListenAudio(document.form_NT_Chapters_mp3.NT_Chapters_mp3, true, \"NTListenNow\", $OTNT)'><span class='lineAction'>".translate('Listen', $st, 'sys')."</span></a> ".translate('to the audio New Testament:', $st, 'sys');
					?>
					...
					foreach ($NT_array[NT_EngBook] as $a) {											// display the NT books in the MAJOR language!
						...
						if ($result_array && $num_array > 0) {
							...
							while ($r_array = $result_array->fetch_array()) {						// display the chapters
								...
								if (!empty($NT_Audio_Filename)) {
									$NT_Audio_Chapter = trim($r_array['NT_Audio_Chapter']);
									echo "<option id='NT_Audio_Chapters_$i' name='NT_Audio_Chapters_$i' class='selectOption' value='$a^https://s3.amazonaws.com/scriptureearth/data/$ISO/audio/$NT_Audio_Filename'>$NT_Audio_Chapter</option>";
								}
								$i++;
							}
							break;
						...
						foreach ($OT_array[OT_EngBook] as $a) {																// display Eng. NT books
							...
							if ($result_array && $num_array > 0) {
								...
								while ($r_array = $result_array->fetch_array()) {						// display the chapters
									...
									//$file_headers = @get_headers("https://s3.amazonaws.com/scriptureearth/data/$ISO/audio/$OT_Audio_Filename");							// Fetches all 9 of the headers sent by the server in response to an HTTP request.
									//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
						...
						foreach ($NT_array[NT_EngBook] as $a) {											// display Eng. NT books
							...
							if ($result_array && $num_array > 0) {
								...
								while ($r_array = $result_array->fetch_array()) {						// display the chapters
									...
									//$file_headers = @get_headers("https://s3.amazonaws.com/scriptureearth/data/$ISO/audio/$NT_Audio_Filename");							// Fetches all 9 of the headers sent by the server in response to an HTTP request.
									//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
...
if ($PlaylistVideo) {															//  test for $Internet is 1 1/2 screens down
	...
	while ($r_Playlist = $result_Playlist->fetch_array()) {
		...
		if (substr($PlaylistVideoFilename, 0, strlen('ScriptureAnim')) == 'ScriptureAnim' || substr($PlaylistVideoFilename, 0, strlen('scripture-videos')) == 'scripture-videos') {
			$tempROD_Code = $ROD_Code;
			$tempVariant_Code = $Variant_Code;
			if (is_null($tempVariant_Code) || !isset($tempVariant_Code) || empty($tempVariant_Code)) $tempVariant_Code = '';
			while (strlen($tempROD_Code) < 5) {
			...
			if (substr($PlaylistVideoFilename, 0, strlen('scripture-videos')) == 'scripture-videos') {
				$filename = "../data/$ISO/video/$ISO" . ($tempROD_Code == '00000' ? '' : '-' . $tempROD_Code) . ($tempVariant_Code == '' ? '' : '-' . $tempVariant_Code) . "-scripture-videos." . $MajorCountryAbbr . '.txt';
				if (file_exists($filename)) {
				//$file_headers = @get_headers($filename);							// Fetches all 9 of the headers sent by the server in response to an HTTP request.
				//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
					$PlaylistVideoFilename = "$ISO" . ($tempROD_Code == '00000' ? '' : '-' . $tempROD_Code) . ($tempVariant_Code == '' ? '' : '-' . $tempVariant_Code) . "-scripture-videos." . $MajorCountryAbbr . '.txt';
				}
			}
			else if (substr($PlaylistVideoFilename, 0, strlen('ScriptureAnim')) == 'ScriptureAnim') {
				$filename = "../data/$ISO/video/ScriptureAnim-$ISO" . ($tempROD_Code == '00000' ? '' : '-' . $tempROD_Code) . ($tempVariant_Code == '' ? '.' : '-' . $tempVariant_Code . '.') . $MajorCountryAbbr . '.txt';
				if (file_exists($filename)) {
				//$file_headers = @get_headers($filename);							// Fetches all 9 of the headers sent by the server in response to an HTTP request.
				//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
					$PlaylistVideoFilename = "ScriptureAnim-$ISO" . ($tempROD_Code == '00000' ? '' : '-' . $tempROD_Code) . ($tempVariant_Code == '' ? '.' : '-' . $tempVariant_Code . '.') . $MajorCountryAbbr . '.txt';
				}
			}
			else {
			}
		...
		************************************************************************/
		...
		if (substr($PlaylistVideoFilename, 0, strlen('Magdalena')) == 'Magdalena') {
			$temp = preg_match("/\t(.*) — /", $VideoConvertContents[0], $match);	// first word of the first line (Magdalena, etc.)
			if (!$temp) {
				preg_match("/\t(.*)\t/", $VideoConvertContents[0], $match);
			}
			$PlaylistVideoTitle = $match[1];
		}
		if (substr($PlaylistVideoFilename, 0, strlen('scripture-videos')) == 'scripture-videos' || substr($PlaylistVideoFilename, 0, strlen('ScriptureAnim')) == 'ScriptureAnim') {
			$temp = preg_match("/\t(.*) — /", $VideoConvertContents[0], $match);	// first word of the first line (ScriptureAnim, etc.)
			if (!$temp) {
				preg_match("/\t(.*)\t/", $VideoConvertContents[0], $match);
			}
			$PlaylistVideoTitle = $match[1];
            ...
            <td>
                ...
                if ($BibleStory != 0) {
                    ...
                    if (stripos($VideoConvertWithTab[3], 'http', 0) === 0) {									// returns 0 means 'http' starts at column 0. === needs to be this way (and not ==) because jPlayer wont work.
                    	echo "<div style='text-align: center; '><a href='#' style='text-align: center; ' title='".$VideoName."'";
                        //$z++;
...
/*                    }
					...
					if ($image != 2) {																			// != 2 ScriptureAnim to make sure the first picture is skipped
						if ($image == 1) {
							echo "<img src=https://s3.amazonaws.com/scriptureearth/data/~images/$bookName/";
						}
						else if ($image == 0) {
							echo "<img src='https://s3.amazonaws.com/scriptureearth/data/$ISO/video/";
						}
						else {
						}
					...
					<tr style="margin-top: 8px; margin-bottom: 8px; ">
						...
						for ($a = $i; $i < count($VideoConvertContents); $i++, $c++) {							// continue using $i to iterate through $VideoConvertContents
							...
							echo '<td style="width: 25%; text-align: center; vertical-align: top; ">';
								echo "<a href='#' title='".$VideoConvertWithTab[1]."'";
								if (stripos($VideoConvertWithTab[3], 'http', 0) === 0) {						// returns 0 means 'http' starts at column 0. === needs to be this way (and not ==) because jPlayer wont work.
								...
								if ($image == 1) {
									...
									echo "<img src='https://s3.amazonaws.com/scriptureearth/data/~images/ScriptureAnim/";
								}
								...
								var myPlaylist_<?php echo $SEVideoPlaylist; ?> = new jPlayerPlaylist({ 			// Create a var containing the playlist object
										...
										foreach ($SEVideoPlaylistArray as $key => $SEVP) {
											...
											?>
											{
												...
												poster: "https://s3.amazonaws.com/scriptureearth/data/~images/ScriptureAnim/<?php echo $pieces[1]; ?>"
											},
											<?php
											$j++;
										}
...
if ($PlaylistVideo) {																						//  test for $Internet is 1 1/2 screens down
	...
	while ($r_Playlist = $result_Playlist->fetch_array()) {
		...
		if (strpos($PlaylistVideoFilename, 'ScriptureAnim') !== false || substr($PlaylistVideoFilename, 0, strlen('scripture-videos')) == 'scripture-videos') {
			$tempROD_Code = $ROD_Code;
			$tempVariant_Code = $Variant_Code;
			if (is_null($tempVariant_Code) || !isset($tempVariant_Code) || empty($tempVariant_Code)) $tempVariant_Code = '';
			while (strlen($tempROD_Code) < 5) {
			...
			else if (strpos($PlaylistVideoFilename, 'ScriptureAnim') !== false) {
				$filename = "../data/$ISO/video/$ISO" . ($tempROD_Code == '00000' ? '' : '-' . $tempROD_Code) . ($tempVariant_Code == '' ? '' : '-' . $tempVariant_Code) . '-ScriptureAnim-640.' . $MajorCountryAbbr . '.txt';
				if (!file_exists($filename)) {
				//$file_headers = @get_headers($filename);							// Fetches all 9 of the headers sent by the server in response to an HTTP request.
				//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
					$PlaylistVideoFilename = $ISO . ($tempROD_Code == '00000' ? '' : '-' . $tempROD_Code) . ($tempVariant_Code == '' ? '' : '-' . $tempVariant_Code) . '-ScriptureAnim-640.' . $MajorCountryAbbr . '.txt';
				}
			}
			else {
			}
		...
		//$file_headers = @get_headers($filename);							// Fetches all 9 of the headers sent by the server in response to an HTTP request.
		//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
			// The text file $filename does not exist.
			continue;
		}
		...
		if (stripos($tempArray[3], 'http', 0) === 0) {														// returns 0 means 'http' starts at column 0. === needs to be this way (and not ==) because jPlayer wont work.
			continue;
		}
		
		// first word of the first line (The Jesus Film, etc.) of txt file
		if (substr($PlaylistVideoFilename, 0, strlen('JesusFilm')) == 'JesusFilm' || substr($PlaylistVideoFilename, 0, strlen('JESUSFilm')) == 'JESUSFilm') {
			$temp = preg_match("/\t(.*) — /", $VideoConvertContents[0], $match);							// first word of the first line (The Jesus Film, etc.) of txt file
			if (!$temp) {
				preg_match("/\t(.*)\t/", $VideoConvertContents[0], $match);
			}
			$PlaylistVideoTitle = $match[1];
		...
		if (substr($PlaylistVideoFilename, 0, strlen('Magdalena')) == 'Magdalena') {
			$temp = preg_match("/\t(.*) — /", $VideoConvertContents[0], $match);							// first word of the first line (Magdalena, etc.) of txt file
			if (!$temp) {
				preg_match("/\t(.*)\t/", $VideoConvertContents[0], $match);
			}
			$PlaylistVideoTitle = $match[1];
		...
		if (substr($PlaylistVideoFilename, 0, strlen('scripture-videos')) == 'scripture-videos' || substr($PlaylistVideoFilename, 0, strlen('ScriptureAnim')) == 'ScriptureAnim') {
			$temp = preg_match("/\t(.*) — /", $VideoConvertContents[0], $match);							// first word of the first line (ScriptureAnim, etc.) of txt file
			if (!$temp) {
				preg_match("/\t(.*)\t/", $VideoConvertContents[0], $match);
			}
			$PlaylistVideoTitle = $match[1];
		...
		<tr>
             <td style='width: 45px; '>
				<?php
				echo "<a href='#' title='".translate('Download', $st, 'sys')." $PlaylistVideoTitle' onclick='PlaylistVideo(orgVideoPixels_$z, \"PlaylistVideoDownload_$z\", $mobile)'><img class='iconActions' src='images/MP4-icon.jpg' alt='".translate('Download', $st, 'sys')."' title='".translate('Download', $st, 'sys')."' /></a>";
				?>
			</td>
			<td>
				<?php
				echo "<a href='#' title='".translate('Download', $st, 'sys')." $PlaylistVideoTitle' onclick='PlaylistVideo(orgVideoPixels_$z, \"PlaylistVideoDownload_$z\", $mobile)'><span class='lineAction'>".translate('Download', $st, 'sys')."</span> $PlaylistVideoTitle</a>";
                ...
                <table id='PlaylistVideoDownloadTable_<?php echo $z; ?>' style='margin-bottom: 15px; width: 100%; '>
                    <tr>
                        <td colspan='4' style='width: 100% '>
                            <?php
                            echo "<input type='button' style='float: right; margin-top: 0px; margin-right: 20px; font-size: 11pt; font-weight: bold; ' value='".translate('Download selected videos', $st, 'sys')."' onclick='DownloadVideoPlaylistZip(\"$st\", \"$ISO\", \"$PlaylistVideoFilename\", \"$z\", \"$mobile\", \"".translate('Please wait!<br />Creating the ZIP file<br />which will take a while.', $st, 'sys')."\")' />";
                            ?>
                            <div id='PlaylistVideoDownload_MB' style='float: right; vertical-align: bottom; margin-top: 6px; '></div>
                        </td>
                    </tr>
                    <?php
                    $media_index = 4;
                    ...
                    <tr>
                        ...
                        for (; $i < count($VideoConvertContents); $i++) {										// continue using $i to iterate through $VideoConvertContents
                            ...
                            <td colspan='<?php echo $col_span; ?>' style='vertical-align: top; width: <?php echo $num_array_col; ?>; '>
								...
								//$file_headers = @get_headers($VideoPlaylist_Download_Filename);					// Fetches all 9 of the headers sent by the server in response to an HTTP request.
								//if ($file_headers[0] != 'HTTP/1.1 404 Not Found' && $file_headers[0] != 'HTTP/1.1 302 Moved Temporarily') {
                                ...
                                echo "<input type='checkbox' id='DVideoPlaylist_$video_download_index' name='DownloadVideoPlaylist_$z' value='$video_download_index' onclick='DownloadVideoPlaylistClick(\"$video_download_index\", $ZipFile)' />&nbsp;&nbsp;<span style='font-size: 11pt; font-weight: normal; '>$VideoConvertWithTab[1]</span>";
                                ?>
                            </td>
                            <?php
                            $j++;
