...
if (strlen($iso) == 3) {																		// if not, exit this ISOShow.php script
	...
	// display ROD Code
	echo "<div style='position: relative; top: -120px; left: 240px; z-index: 5; display: inline; font-size: 11pt; font-weight: bold; '>ROD Codes:";	// dope-down dispay of the ROD Code and discriptions
	...
	$num = $result->num_rows;
	if ($num > 0) {																// There is 1 or more ROD dialects
	...
	if ($num > 0) {																// There is 1 or more ROD dialects
		while ($r = $result->fetch_assoc()) {
			...
			$dialect_name=addslashes($dialect_name);
			$location=$r['location'];
			$location=addslashes($location);
			//echo '$rod='.$rod.'; $language_name='.$language_name.'; $dialect_name='.$dialect_name.'; $location='.$location.'<br />';
