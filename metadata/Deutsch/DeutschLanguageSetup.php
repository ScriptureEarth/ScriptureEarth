...
<html>
<head>
<title>Deutsch Language Setup</title>
...
<head>
<title>Scripture Earth Deutsch Language Setup</title>
<meta http-equiv="Content-Type" 			content="text/html; charset=utf-8" />
<meta name="ObjectType" 					content="Document" />
<meta http-equiv="Window-target" 			content="_top" />
...
<meta name="Keywords" lang="en-US" 			content="
		moderne indigene Sprachen, der amerikanische Kontinent, die Welt, Herzenssprache, Muttersprache, Bible.is, online viewer,
		herunterladen, Muttersprache, Text, PDF, Hörversion, MP3, mp3, MP4, mp4, iPod, iPhone, Mobiltelefon, Smartphone, iPad,
		Tablet, Android, ansehen, ansehen, Jesusfilm, Lukasfilm, kaufen, print-on-demand, online kaufen, Buchladen, studieren,
		Das Wort, Bibel, Neues Testament, NT, Altes Testament, AT, Schrift, Landkarte, mobile App,
		#kkkkkkkkkk#
	';
$fileSecondPart = '';
$fileThirdPart = '" />
...
$fileFifthPart = '</div>
	...
	if ($Display) {
		...
		echo "<h2 style='text-align: center; margin: 0px; color: navy; '>Scripture Earth Deutsch Language Setup</h2><br /><br />";
	}
	...
	while ($row = $result->fetch_array()) {
		...
		if (!$LN_German) {								// if the German then the default langauge
			switch ($def_LN){
				case 1:
	...
	$query='SELECT countries.German FROM ISO_countries, countries WHERE ISO_countries.ISO = ? AND ISO_countries.ROD_Code = ? AND ISO_countries.ISO_countries = countries.ISO_Country ORDER BY countries.German';
	...
	while ($row = $result->fetch_array()) {
		...
		$fileFourthPart .= "<tr valign='bottom' style='color: navy; background-color: #$color; margin: 0px; padding: 0px; '>" . PHP_EOL .
			"<td width='28%' style='background-color: #$color; margin: 0px; padding: 3px 5px 3px 5px; border-width: thin; border-style: none; border-color: #$color; '><a href='../../00deu.php?sortby=lang&iso=$ISO&rod=$ROD_Code&var=$Variant_Code'>$LN</a></td>" . PHP_EOL;
		...
		$Germ_country = $r_ISO_C['German'];						// first name of the country in the language version
		while ($r_ISO_C = $result_ISO_countries->fetch_array()) {
			$Germ_country = $Germ_country.', '.$r_ISO_C['German'];	// name of the country in the language version
		}
		if ($Display) {
			...
			echo "<td width='26%' style='margin: 0px; padding: 3px 5px 3px 5px; border-width: thin; border-style: none; border-color: #$color; '>$Germ_country</td>";
			echo '</tr>';
		}
		$fileFourthPart .= "<td width='26%' style='margin: 0px; padding: 3px 5px 3px 5px; border-width: thin; border-style: none; border-color: #$color; '>$Germ_country</td>" . PHP_EOL .
			'</tr>' . PHP_EOL;
