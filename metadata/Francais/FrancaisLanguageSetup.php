...
<head>
<title>Francais Language Setup</title>
...
<meta name="Description" lang="en-US" 		content="
	  Ce site donne accès à la #llllllllll# Bible / NT dans le texte (PDF), audio (MP3), montre (le film Jésus, etc),
	  vous pouvez acheter (impression à la demande), bureau (La Parole), d\'autres livres et liens dans les langues autochtones des Amériques.
  " />
<meta name="Keywords" lang="en-US" 			content="
	  moderne des langues autochtones, Amériques, la langue du cœur, langue maternelle, le texte (PDF), audio (MP3),
	  montre (le film Jésus, etc), vous pouvez acheter (impression à la demande, achat en ligne, librairie),
	  étude (La Parole), la Bible, Nouveau Testament, NT, Ancien Testament, OT,
	  #kkkkkkkkkk#
	  ';
$fileSecondPart = '';
$fileThirdPart = '" />
...
$fileFifthPart = '</div>
	...
	if ($Display) {
		...
		echo "<h2 style='text-align: center; margin: 0px; color: navy; '>Scripture Earth Francais Language Setup</h2><br /><br />";
	}
	...
	$stmt_LN_Gerrman=$db->prepare($query);																// create a prepared statement
	...
	if ($Display) {
		...
		echo "<th width='28%'>Nom de langue:</th>";
		echo "<th width='31%'>Autre nom pour la langue:</th>";
		echo "<th width='15%'>Code de la langue:</th>";
		echo "<th width='26%'>País:</th>";
	}
	$fileFourthPart .= PHP_EOL . "<table border='0' style='width: 980px; margin: 0px; padding: 0px; '>" . PHP_EOL .
		...
		"<th width='28%'>Nom de langue:</th>" . PHP_EOL .
		"<th width='31%'>Autre nom pour la langue:</th>" . PHP_EOL .
		"<th width='15%'>Code de la langue:</th>" . PHP_EOL .
	...
	$query='SELECT countries.French FROM ISO_countries, countries WHERE ISO_countries.ISO = ? AND ISO_countries.ROD_Code = ? AND ISO_countries.ISO_countries = countries.ISO_Country ORDER BY countries.French';
	$stmt_countries_Fre=$db->prepare($query);															// create a prepared statement
	while ($row = $result->fetch_array()) {
		if ($i % 2)
			$color = 'ffffff';
		else
		...
		$fileFourthPart .= "<tr valign='bottom' style='color: navy; background-color: #$color; margin: 0px; padding: 0px; '>" . PHP_EOL .
			"<td width='28%' style='background-color: #$color; margin: 0px; padding: 3px 5px 3px 5px; border-width: thin; border-style: none; border-color: #$color; '><a href='../../00f-Ecritures_Indice.php?sortby=lang&iso=$ISO&rod=$ROD_Code&var=$Variant_Code'>$LN</a></td>" . PHP_EOL;
		...
		//$query="SELECT countries.French FROM ISO_countries, countries WHERE ISO_countries.ISO = '$ISO' AND ISO_countries.ROD_Code = '$ROD_Code' AND ISO_countries.ISO_countries = countries.ISO_Country ORDER BY countries.French";
		//$result_ISO_countries=$db->query($query);
		//$num_ISO_countries=$result_ISO_countries->num_rows;
		$stmt_countries_Fre->bind_param('ss', $ISO, $ROD_Code);											// bind parameters for markers
		$stmt_countries_Fre->execute();																	// execute query
		$result_ISO_countries = $stmt_countries_Fre->get_result();										// instead of bind_result (used for only 1 record):
		//$num_ISO_countries=$result_ISO_countries->num_rows;
		$r_ISO_C = $result_ISO_countries->fetch_array();
		$French_country = $r_ISO_C['French'];					// first name of the country in the language version
		while ($r_ISO_C = $result_ISO_countries->fetch_array()) {
			$French_country = $French_country.', '.$r_ISO_C['French'];					// name of the country in the language version
		}
		if ($Display) {
			...
			echo "<td width='26%' style='margin: 0px; padding: 3px 5px 3px 5px; border-width: thin; border-style: none; border-color: #$color; '>$French_country</td>";
			echo '</tr>';
		}
		$fileFourthPart .= "<td width='26%'style='margin: 0px; padding: 3px 5px 3px 5px; border-width: thin; border-style: none; border-color: #$color; '>$French_country</td>" . PHP_EOL .
			'</tr>' . PHP_EOL;
