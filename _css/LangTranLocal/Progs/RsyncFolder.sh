...
then
    FOLDERSUPDATED=$(sed -n "s/[^0-9]*\([0-9]*\)[^0-9]*/\1/p" Progs/$FolderCount)
else
...
then
    ...
    if $(mkdir -p $Dest)
    then
		echo "Making the destination folder \"$Dest\"."
    else
		echo "Was not able to create a folder \"$Dest\"" >&2
